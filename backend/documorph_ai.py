...
# Import PaddleOCR support
try:
    import sys
    # Add version1.0.0 to path
    version_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "version1.0.0")
    if version_dir not in sys.path:
        sys.path.append(version_dir)
    from paddle_ocr import process_image_with_paddle_ocr, process_figure_with_paddle_ocr
    PADDLE_OCR_AVAILABLE = True
    logging.info("PaddleOCR support is available")
except ImportError:
    PADDLE_OCR_AVAILABLE = False
    logging.warning("PaddleOCR is not available. Install with 'pip install paddlepaddle paddleocr'")
    ...
    try:
        ...
        if UNSTRUCTURED_AVAILABLE:
            ...
            if error is None and chunks:
                # Process images with PaddleOCR if available
                if PADDLE_OCR_AVAILABLE and figures:
                    logger.info(f"Processing {len(figures)} figures with PaddleOCR")
                    for i, figure in enumerate(figures):
                        if "image_bytes" in figure and figure["image_bytes"]:
                            # Extract text from figure using PaddleOCR
                            ocr_text = process_figure_with_paddle_ocr(figure)
                            if ocr_text and ocr_text != "No text detected in the image" and not ocr_text.startswith("Error"):
                                # Add OCR text to figure
                                figures[i]["ocr_text"] = ocr_text
                                # Check if figure is in chunks and update
                                for j, chunk in enumerate(chunks):
                                    if chunk.page_content.startswith("[FIGURE") and figure["figure_id"] in chunk.page_content:
                                        # Update with OCR text
                                        chunks[j].page_content = f"[FIGURE {figure['figure_id']}]\n{ocr_text}\n[/FIGURE]"
                                        break
        ...
        if file_type == "pdf":
            ...
            tables = []
            figures = []
        elif file_type in ["png", "jpg", "jpeg", "tiff"] and PADDLE_OCR_AVAILABLE:
            # For image files, use PaddleOCR directly
            ocr_text = process_image_with_paddle_ocr(image_path=file_path)
            
            # Create a simple document with the OCR text
            from langchain_core.documents import Document
            chunks = [Document(page_content=ocr_text, metadata={"source": file_path})]
            
            # Add the image as a figure too
            with open(file_path, "rb") as img_file:
                image_bytes = img_file.read()
                
            # Create figure data
            figure_id = f"figure_1"
            figures = [{
                "figure_id": figure_id,
                "page": 1,
    ...
    try:
        ...
        try:
            if file_type == "pdf":
                ...
                # Use PaddleOCR for image processing if available, otherwise fall back to standard OCR
                if PADDLE_OCR_AVAILABLE:
                    ocr_text = process_image_with_paddle_ocr(image_path=file_path)
                    
                    # Create a simple element with the OCR text
                    from unstructured.documents.elements import NarrativeText
                    elements = [NarrativeText(text=ocr_text)]
                    ...
                    # Fall back to standard unstructured processing with OCR
                    from unstructured.partition.image import partition_image
        ...
        for i, element in enumerate(elements):
            ...
            if isinstance(element, Table):
                ...
                if PADDLE_OCR_AVAILABLE and image_data:
                    ocr_text = process_figure_with_paddle_ocr(fig_data)
                    if ocr_text and ocr_text != "No text detected in the image" and not ocr_text.startswith("Error"):
                        fig_data["ocr_text"] = ocr_text
                        # Use OCR text instead of default text
                        text_to_add = ocr_text
                    else:
                        text_to_add = element.text if hasattr(element, "text") else ""
                else:
                    text_to_add = element.text if hasattr(element, "text") else ""
