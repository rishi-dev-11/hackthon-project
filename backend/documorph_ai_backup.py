...
import fitz  # PyMuPDF # Explicitly import Document - doesn't work with current PyMuPDF
import cv2
import numpy as np
    ...
    def get_tier_features():
        return {
            UserTier.FREE: {
                ...
                "template_categories": ["Student", "Content Creator"],
            ...
            UserTier.PREMIUM: {
                ...
                "llm_enabled": True,
                "multi_language": True,
                "ocr_languages": ["eng", "fra", "deu", "spa", "rus", "ara", "chi_sim", "jpn", "kor"],
...
def detect_tables_from_pdf(pdf_path):
    ...
    try:
        tables = []
        # Use our compatible open_pdf function
        ...
        for page_num, page in enumerate(doc):
            ...
            if tab_dict and tab_dict.tables:
                for i, table in enumerate(tab_dict.tables):
                    ...
                    rows = []
                    for row in cells:
                        # Handle case where row elements could be strings or objects with text attribute
                        row_data = []
                        for cell in row:
                    ...
                    if rows:
                        ...
                        # Store HTML version for web display
                        if df is not None:
                            tables[-1]["html"] = df.to_html(index=False)
                
        logger.info(f"Detected {len(tables)} tables in {pdf_path}")
        return tables
    except Exception as e:
...
def detect_figures_from_pdf(pdf_path):
    ...
    try:
        ...
        logger.info(f"Detected {len(figures)} figures in {pdf_path}")
        return figures
    except Exception as e:
...
def add_figure_to_docx(doc, figure, caption):
    """Add a figure to a DOCX document with proper formatting."""
    try:
...
# Add a function to process documents with unstructured
...
def main():
    ...
    if feature_selection == "Upload & Process Document":
                ...
                if templates:
                    for i, template in enumerate(templates):
                        with st.expander(f"{template['name']}"):
                            ...
                            col1, col2 = st.columns(2)
                            with col1:
                                if st.button("Select Template", key=f"select_template_{i}"):
        ...
        # For Premium users, use LLM; for Free users, use rule-based approaches
        if can_use_llm(st.session_state.user_tier):
            llm = init_llm()
            if not llm:
            ...
            with tab1:
                ...
                instructions = st.text_area("Instructions for abstract (optional)", 
                                          "Generate a concise, professional abstract focusing on the main points.")
            ...
            with tab2:
                ...
                if selected_section != "Select a section":
                    ...
                    instructions = st.text_area("Instructions for title suggestions", 
                                              "Make titles more engaging and specific.")
            ...
            with tab3:
                ...
                target_style = st.selectbox("Target style", 
                                           ["Business Formal", "Academic", "Technical", "Casual Professional", "Creative"])
    ...
    try:
        ...
        credentials = {
            "installed": {
                ...
                "redirect_uris": ["http://localhost", "urn:ietf:wg:oauth:2.0:oob", "http://localhost:8501"]
            }
        }
        ...
        # Create a flow - use the actual redirect URI from Streamlit
        flow = InstalledAppFlow.from_client_config(
            credentials, 
            ['https://www.googleapis.com/auth/drive.file'],
            redirect_uri='http://localhost:8501'  # Use Streamlit's local server
        )
