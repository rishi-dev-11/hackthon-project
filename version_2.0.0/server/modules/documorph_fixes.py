...
def setup_google_drive_auth():
    """Update the Google Drive authentication configuration."""
    try:
        ...
        def create_auth_flow(client_id, client_secret):
            """Create an OAuth flow with the proper redirect URI."""
...
def extract_images_from_pdf(pdf_path, output_folder="extracted_images"):
    """
    Extracts unique images from a PDF and saves them to the specified folder.
    ...
    for page_number in range(len(doc)):
        ...
        for img_index, img in enumerate(images):
            ...
            # Add image info to our result list
            extracted_images.append({
                ...
                "height": height,
                "format": image_ext,
                "caption": f"Figure {img_index+1} from page {page_number+1}"
            })
    
    return extracted_images

def process_extracted_tables(tables, output_format="csv", output_folder="extracted_tables"):
    """
    Process extracted tables and save them in the specified format.
    ...
    for idx, table_data in enumerate(tables):
        ...
        table_filename = f"table_{idx+1}.{output_format}"
        ...
        if output_format == "csv":
            ...
            table_data.to_excel(table_filepath, index=False)
        elif output_format == "html":
            table_data.to_html(table_filepath, index=False)
        else:
            table_data.to_csv(table_filepath, index=False)  # Default to CSV
        
        # Add table info to our result list
...
if __name__ == "__main__":
    st.title("DocuMorph AI - Fixes Demo")
    
    # Create test user
    user_id = str(uuid.uuid4())
    
    with st.expander("MongoDB Template Initialization"):
        if st.button("Initialize Templates"):
            success = initialize_db_templates(user_id)
            if success:
                st.success("Default templates created!")
            else:
                st.error("Failed to create templates")
                
    with st.expander("PDF Processing Demo"):
        ...
        if uploaded_file:
            ...
            figures = extract_figures_from_pdf(temp_path)
            ...
            for i, table in enumerate(tables):
                with st.expander(f"Table {i+1}"):
                    ...
                    elif "dataframe" in table and table["dataframe"] is not None:
                        st.dataframe(table["dataframe"])
                    else:
                        st.write("Raw data:", table["raw_data"])
                        
            st.subheader("Figures")
            for i, figure in enumerate(figures):
                with st.expander(f"Figure {i+1}"):
                    if "preview_src" in figure:
                        st.image(figure["preview_src"])
                    else:
                        st.write("Figure data not available for preview")
