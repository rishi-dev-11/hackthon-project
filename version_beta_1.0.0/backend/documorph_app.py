...
def apply_template_to_document(content_doc, template_config, lang, logo_path=None):
    ...
    # Process content with NLP section detection
    ...
    for paragraph in content_doc.paragraphs:
        if not paragraph.text.strip():
            continue
        if section_index < len(sections) and paragraph.text.strip().lower() == sections[section_index]["text"].lower():
    ...
    for table in content_doc.tables:
        ...
        caption_para = output_doc.add_paragraph(f"{template_config.get('table_caption_prefix', 'Table')} {table_count}: Table")
...
def process_document(uploaded_file, template_config, logo_file=None):
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.docx') as tmp_file:
        ...
        # Translate template text if not English
        if lang != 'en':
            model, tokenizer = load_translation_model()
            text_fields = ['header_text', 'footer_text', 'document_title', 'document_author']
            for field in text_fields:
                if field in template_config:
                    template_config[field] = translate(template_config[field], 'en', lang, model, tokenizer)
            if 'figure_caption_prefix' in template_config:
                template_config['figure_caption_prefix'] = translate(template_config['figure_caption_prefix'], 'en', lang, model, tokenizer)
            else:
                template_config['figure_caption_prefix'] = translate('Figure', 'en', lang, model, tokenizer)
            if 'table_caption_prefix' in template_config:
                template_config['table_caption_prefix'] = translate(template_config['table_caption_prefix'], 'en', lang, model, tokenizer)
            else:
                template_config['table_caption_prefix'] = translate('Table', 'en', lang, model, tokenizer)
        ...
        if template_action == "Select Template":
            ...
            if templates_collection:
                ...
                if template_names:
                    ...
                    selected_template = next((t for t in user_templates if t["name"] == selected